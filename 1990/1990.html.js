#!/usr/bin/env node
"use strict";var V=Object.create;var C=Object.defineProperty;var Q=Object.getOwnPropertyDescriptor;var Z=Object.getOwnPropertyNames;var K=Object.getPrototypeOf,X=Object.prototype.hasOwnProperty;var tt=(t,e,o,s)=>{if(e&&typeof e=="object"||typeof e=="function")for(let n of Z(e))!X.call(t,n)&&n!==o&&C(t,n,{get:()=>e[n],enumerable:!(s=Q(e,n))||s.enumerable});return t};var et=(t,e,o)=>(o=t!=null?V(K(t)):{},tt(e||!t||!t.__esModule?C(o,"default",{value:t,enumerable:!0}):o,t));var u=et(require("fs")),f=(t,e)=>u.writeFileSync(t,e,{encoding:"utf8"}),l=t=>u.readFileSync(t,{encoding:"utf8"});var L=new Date,x=`v3.1.0-${L.toJSON()}`,r=btoa(`${Number(L)}`).slice(10,18);var b=t=>`<a ${/^http/.test(t.href)?' rel="noopener noreferrer"':""} href="${t.href}" title="${t.title}">${t.title}</a>`;var y=t=>t.replace(/[&<"']/g,e=>{switch(e){case"&":return"&amp;";case"<":return"&lt;";case'"':return"&quot;";default:return"&#039;"}});var w=({src:t})=>`<pre><font size=1>${y(l(t))}</font></pre>`;var P=({src:t,title:e})=>`<fieldset bgcolor="green">
                                                <legend>${e}</legend>
                                                <code width="120px">${y(l(t))}</code>
                                            </fieldset>`;var a=({srcDoc:t,src:e,nonce:o,async:s,prefix:n,postfix:c,iife:g,crossorigin:v="",integrity:m})=>{let h=`${n||""}${t?l(t):""}${c||""}`;return`<script ${s?"async":""} crossorigin="${v}" ${m?`integrity="${m}"`:""} nonce="${o}" ${e?`src="${e}"`:""}>${g?`(${h})(${g})`:h}</script>`};var E=({nonce:t,gtmId:e})=>`
...
`})}
`;var I=({nonce:t,integrity:e,projectId:o,...s})=>`
    ${a({async:!0,nonce:t,src:"https://browser.sentry-cdn.com/8.29.0/bundle.tracing.min.js",integrity:e})}
    ${a({srcDoc:"./src/lib/sentry.js",nonce:t,iife:JSON.stringify(s)})}
`;var G=({name:t,rows:e,cols:o,data:s,type:n,value:c="",required:g,l10n:v})=>{let m=t,h=g?"required":"";return s?`<select id="${m}" name="${t}" ${h}>
                                                              <option></option>
                                                              ${s.map(M=>`<option value="${M}">${v(M)}</option>`).join(`
`)}
                        ...
                        <a id="toggleDosStyle" href="#exit" bgcolor="red">DOS</a>
                                        ...
                                        <font color="#DA214C">Nest</font>`,p=[{image:{ascii:"./src/pages/1990/assets/website.ascii"},description:"Website development, including complex forms with custom field validation, popups and dialogs.",links:[{href:"https://wiam-front-test.vercel.app/",title:"Ex. 1"},{href:"https://mindmap-opal.vercel.app/",title:"Ex. 2"},{href:"https://codepen.io/iegik/full/ObZpqo",title:"Ex. 3"},{href:"https://test-kanvajs.vercel.app/",title:"Ex. 4"}]},{image:{ascii:"./src/pages/1990/assets/mobile.ascii"},description:"Mobile applications - developing, publishing or upgrading existing ones.",links:[{href:"https://github.com/iegik/inventarizacija/",title:"Ex. 1"},{href:"https://github.com/iegik/react_calc/",title:"Ex. 2"}],comment:"Unfortunately apps are not available in Google Play anymore"},{image:{ascii:"./src/pages/1990/assets/extensions.ascii"},description:"Browser extensions, custom scripts and other researches where I'm testing new approaches.",links:[{href:"https://github.com/iegik/clock-extension",title:"Ex. 1"},{href:"https://github.com/iegik/thunar-scripts",title:"Ex. 2"},{href:"https://hub.docker.com/repository/docker/iegik/docker-node/general",title:"Ex. 3"}]},{image:{ascii:"./src/pages/1990/assets/cms.ascii"},description:"CMS (Wordpress, Magento) configuration, plugin creation, RESTFul API on PHP, GraphQL and microservices",links:[{href:"https://github.com/WinLinMac/magento_themes",title:"Ex. 1"}]}],B=[{link:{href:"https://t.me/ajansons",title:"t.me/ajansons"}}],N=[{href:"https://linkedin.com/in/iegik",title:"LinkedIn"},{href:"https://github.com/iegik",title:"GitHub"},{href:"https://profile.codersrank.io/user/iegik",title:"CodersRank"},{href:"https://codepen.io/iegik/",title:"CodePen"},{href:"https://jsfiddle.net/user/iegik",title:"JSFiddle"},{href:"https://stackoverflow.com/users/771471/iegik",title:"StackOverflow"}],O="'SFMono-Regular', 'SF Mono', 'Ubuntu Mono', Consolas, 'DejaVu Sans Mono', Menlo, monospace",d=t=>`<center>${w({src:t.image.ascii})}</center>
                                            ...
                                            ${t.comment?`<p><em><font color="gray">${t.comment}</font></em>`:""}`,H=()=>"<p>Time Zone: EEST</p>",U=()=>"<p>Work Time: 10:00 - 20:00</p>",W=()=>"<p>Current Location: Earth</p>",R={email:{kind:"vertical",required:!0},to:{type:"hidden",value:"a.jansons+web@gmail.com"},access_key:{type:"hidden",value:"c5540606-b7ca-4634-980a-13e2c50cd823"},redirect:{type:"hidden",value:"/1990/sent"},subject:{kind:"vertical",required:!0,data:["feedback","work","consultation","issue"]},message:{kind:"vertical",required:!0},send:{type:"submit"}},st={email:"Your email",subject:"Subject",message:"Content",send:"Send message",feedback:"Feedback",work:"Work opportunity",consultation:"Consultation",issue:"Bug Report",botcheck:"I' m not a robot"},k=t=>st[t]||t,at=()=>t=>e=>{let{name:o}=e,{kind:s,type:n}=R[o]||{},c={...e,...R[o]||{},l10n:k};if(n==="hidden")return t(c);if(n==="submit")return`<input type="submit" name="${o}" value="${k(o)}" />`;let g=k(o);return s==="vertical"?`<label for="${o}">${g}</label>
                                                        <br>
                                                        <br>
                                                        ${t(c)}`:`<label for="${o}">${g}</label>&nbsp;${t(c)}`},i=at()(G),J=({name:t})=>`<input id="bot" name="${t}" nonce="${r}" value="This shouldn't be here" /><p><a id="${t}" href="#bot">[_] ${k(t)}</a></p><script nonce="${r}">bot.style.display='none';${t}.addEventListener("click", () => {${t}.innerText=${t}.innerText.slice(0,1)+(bot.value==='true' ? '_' : 'x')+${t}.innerText.slice(2);bot.disabled=true;bot.value=bot.value === 'true' ? false : true;});</script>`,ct=()=>`<form align="left" method="POST" action="https://api.web3forms.com/submit">
...
<head>
    ...
    <style nonce="${r}" id="/1990/styles.min.css">${ot}</style>
    
    ${E({nonce:r,gtmId:"GTM-MBG56M"})}
    
    
</head>
<body>
    
    ${z({nonce:r,gtmId:"G-5ZY8Y6X2C4"})}
    
    ${D({nonce:r,gtmId:"GTM-MBG56M"})}
    ${t}
...
`);f("public/1990/desktop/index.html",Y(pt()));f("public/1990/mobile/index.html",Y(dt()));var gt=`
CACHE MANIFEST
# rev ${x}
