...
`,h=de;var ge=(e={})=>{let{className:t="link",children:o,tag:r="a",to:s="#",title:n="",onClick:i}=e,g=d();return setTimeout(()=>{i&&g.current&&g.current.addEventListener("click",i)}),`
      <${r} class="${t} link" href="${s}" title="${n}" ref="${g}">
        ${o?.join?.("")}
      </${r}>
    `},a=ge;var H=e=>h({message:e.message,children:[a({to:"#/login",title:"Go to login page",children:["Log in"]})]});var f="ae0dcffd947e0ca15a12",x="user repo",D="Access token not provided",p="Page not found",O="Wrong request";var B=(e={message:p})=>h({message:e.message,children:[a({to:"#",title:"Go to home page",children:["Home"]}),a({onClick:()=>{history.go(-1)},title:"Go back",children:["Back"]})]});var G=e=>h({message:e.message,children:[a({onClick:()=>{location.reload()},title:"Reload",children:["Reload"]}),a({onClick:()=>{history.go(-1)},title:"Go back",children:["Back"]})]});var E=!0,y=e=>{if(typeof window=="undefined")return;E&&typeof Sentry!="undefined"&&Sentry.captureException(e);let t=document.getElementById("root");if(t!=null){if(e.message===p){t.innerHTML=B(e);return}if(e.message===D){t.innerHTML=H(e);return}t.innerHTML=G(e)}},L=E?()=>{}:(...e)=>{console.debug(...e)};var c=E?()=>{}:(...e)=>{console.info(...e)};var w=e=>{let{protocol:t,hash:o,pathname:r}=new URL(`${e?.destination?.url||document.location}`);return`${r.replace("index.html","")}${o.slice(2)}`},z=e=>e.replace(/[&<"']/g,t=>{switch(t){case"&":return"&amp;";case"<":return"&lt;";case'"':return"&quot;";default:return"&#039;"}});var fe=async e=>{if(!e)return;L("Requesting access_token",{code:e,clientId:f});let{access_token:t,scope:o,token_type:r}=await fetch(`https://qilg4ch66b3vpgtevzccb5meum0ttfcl.lambda-url.eu-north-1.on.aws/?code=${e}&client_id=${f}`).then(s=>s.json());window.sessionStorage?.setItem("access_token",t),window.sessionStorage?.setItem("scope",o),window.sessionStorage?.setItem("token_type",r),document.location.href="/#/profile"},ue=async e=>{let{protocol:t,hash:o,search:r}=location,s=window.sessionStorage?.getItem("state"),n=window.sessionStorage?.getItem("code");L("Checking GET params",{protocol:t,hash:o,search:r,state:s});let i=new URLSearchParams(r);if(i.get("state")!==s)return window.history?.go(-2);fe(i.get("code")||void 0)},K=ue;var J=`query RepoFile($repo_owner: String!, $repo_name: String!, $object_path: String!) {
...
`;var he=async e=>{let{data:t,errors:o}=await e.json();if(o)throw o[0];return t},I=class{constructor(){this.repoName="iegik.github.com";this.repoOwner="iegik";this.storeRoot="gh-pages:data"}static getInstance(){return this.instance}async request({query:t,variables:o}){if(typeof window=="undefined")return;let r=window.sessionStorage?.getItem("access_token"),s=window.sessionStorage?.getItem("token_type"),n=await fetch("https://api.github.com/graphql",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json",Authorization:`${s} ${r}`},body:JSON.stringify({query:t,variables:o})});return he(n)}async getFile(t){let o=await this.request({query:J,variables:{repo_name:this.repoName,repo_owner:this.repoOwner,object_path:`${this.storeRoot}${t}.json`}});if(!o)throw Error(O);let{repository:{object:r}}=o;if(!r)throw Error(p);let{text:s}=r;return JSON.parse(s)}async getUserInfo(){let t=await this.request({query:V,variables:{login:this.repoOwner}});if(!t)throw Error(O);return t}},S=I;S.instance=new I;var b=S;var we=async e=>{let[{isLoaded:t},o]=u();t||(async()=>{let s=w(),n=await b.getInstance().getFile(s);o({data:n,isLoaded:!0})})()},Q=we;var ve=e=>e.json(),T=class{constructor(){}static getInstance(){return this.instance}async request(t){let o=await fetch(`${location.protocol}//${location.host}/data${t}`,{headers:{"Content-Type":"application/json",Accept:"application/json"}});return ve(o)}async getFile(t){let o=await this.request(`${t}.json`);if(!o)throw Error(p);return o}},N=T;N.instance=new T;var X=N;var ke=async e=>{let[t={},o]=u();c("PreviewService",{state:t,setUserInfo:o});let r=w(),s=await X.getInstance().getFile(r);c(`File ${r} loaded`,{data:s,state:t}),o(s)},Y=ke;var $e=async e=>{let[{avatarUrl:t},o]=u();t||(async()=>{let{avatarUrl:s,login:n}=await b.getInstance().getUserInfo();o({children:[{tag:"img",src:s},{tag:"span",children:n}]})})()},W=$e;var xe={oauth:K,editor:Q,preview:Y,userInfo:W},Z=xe;var ee=class{constructor(){this.id=null}toString(){let t=btoa(`${Math.ceil(Math.random()*1e13)+ +new Date}`).slice(10,18);return this.id=this.id||`ref-${t}`}get current(){return typeof window=="undefined"||typeof document=="undefined"?null:document.querySelector(`[ref=${this.id}]`)}},d=()=>new ee,u=e=>typeof history=="undefined"?[]:[e||history.state,(t,o,r)=>history.pushState({...history.state,...t},o||"")],ye={attributes:!0,childList:!0,subtree:!0},be=(e,t)=>{for(let o of e)o.type==="childList"?console.log("A child node has been added or removed.",{ref:o.target.getAttribute("ref")}):o.type==="attributes"&&console.log(`The ${o.attributeName} attribute was modified.`)},j=(e={})=>{let{tag:t="div",className:o="",children:r=[],services:s=[],...n}=e,i=d(),[g,_]=u(),U=m=>{c("popstate",{state:m})};addEventListener("popstate",U),setTimeout(async()=>{let m=async()=>{c("ref",{ref:i,state:history.state,services:s});let k=new MutationObserver(be);k.observe(i.current,ye),i.current.addEventListener("DOMRemoved",()=>{c("DOMRemoved"),removeEventListener("popstate",U),k.disconnect()}),await Promise.all(s?.map(v=>Z[v]?.(i))).catch(v=>{y(v)}),_({services:null}),i.current.innerHTML=j(g)};s?.length&&m()});let q=Array.isArray(r)?r.map(m=>m instanceof Object?j(m):`${m}`).join?.(""):z(r),A=Object.entries(n)?.reduce?.((m,[k,v])=>`${m} ${k}="${v}"`,"");return c("Rendering",{content:q,restProps:A}),`
    <${t} class="${o}" ref="${i}" ${A}>
      ${q}
    </${t}>
  ...
  `).join(""),je=e=>{if(!e)return"";let{src:t,ratio:o,sizes:r,types:s="png|webp",alt:n="",itemprop:i=""}=e,[g,_]=o.split(":");return`
    <picture>
      ${Te({types:s,sizes:r,src:t})}
      <img src="${s.includes("webp")?t.replace(/\..*$/,".webp"):t}" width="${g}" height="${_}" alt="${n}" itemprop="${i}" />
    </picture>
  `},se=je;var ne=`                                    
             ....                   
         ....... .........          
...
`;var Ce=()=>`
  ...
  `,ae=Pe;var Me=(e={})=>{let{className:t="",children:o,tag:r="button",onClick:s,type:n}=e,i=d();return setTimeout(()=>{s&&i.current?.addEventListener("click",s)}),`
    ...
    `},ce=Me;var pe="^[\\d\\w\\-\\.]+$",xo=new RegExp(pe);var Fe=()=>{if(typeof document=="undefined"||typeof window=="undefined"||typeof window.sessionStorage=="undefined")return;let e=document.querySelector('[name="login"]');if(e.validity.valid===!1)return;let t=e?.value,o=btoa(`${+new Date}`).slice(10,18);window.sessionStorage?.setItem("state",o);let r=`https://github.com/login/oauth/authorize?client_id=${f}&login=${encodeURIComponent(t)}&scope=${encodeURIComponent(x)}&state=${o}`;c(`Navigated to ${r}`,{clientId:f,scope:x,state:o}),document.location.href=r,history.pushState({clientId:f,scope:x,state:o},"",r)},Ue=()=>`
    <div class="login">
      <form>
  ...
  `,le=Ue;var R=e=>te({children:["Loading..."],...e}),qe=(e="/")=>{switch(c(`Loading ${e}`),!0){case/^\/login\/?$/.test(e):return le();case/^\/oauth\/?$/.test(e):return R({services:["oauth"]});case/^\/profile\/?$/.test(e):return R({services:["preview"]});case/^\/editor\/?$/.test(e):return R({services:["editor"]});case/^\/preview\/?$/.test(e):return R({services:["preview"]});case(e==="/"||e===""):return ae();default:throw Error(p)}},Ae=e=>{let{error:t}=e;y(t)},He=e=>{let t=w(e);if(typeof window!="undefined"&&window?.document){addEventListener("error",Ae);let o=document.getElementById("root");if(o==null)return;o.innerHTML=qe(t)}};He();})();
