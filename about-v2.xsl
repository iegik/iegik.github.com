<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xdt="http://www.w3.org/2005/02/xpath-datatypes">
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="css/fancybox/jquery.fancybox-1.2.5.css" media="all" />
		...
		<div id="doc2">
			<div id="hd">
				...
				<ul class="navigation">
					...
					<li>
						<a href="portfolio.html" title="My Works">portfolio</a>
					</li>
					<li>
						<a href="twitter.html" title="Twitter">twitter</a>
					</li>
					<li>
						<a href="fancybox.html" title="FancyBox" name="fancybox">fancybox</a>
					</li>
					<li>
						<a href="yui-wizard.html" title="Yahoo User Interfase wizard">Layout</a>
					</li>
					<li>
						<a href="calc.html" title="Currency online calculator">Currency</a>
					</li>
					<li>
						<a href="sudoku.html" title="Sudoku">Sudoku</a>
					</li>
					<li>
						<a href="about.html" title="About Me" class="hightlight current">about</a>
					</li>
				</ul>
			</div>
			...
			<div id="ft" class="footer">
				<p>
					...
					<a rel="in-area" title="WAI Home" href="http://www.w3.org/WAI/"><img alt="Level Double-A conformance icon, W3C-WAI Web Content Accessibility Guidelines 1.0" src="img/wai_aa_wcag10_80x15.gif" /></a>
				</p>
			</div>
		</div>
	</body>
	...
	<xsl:template match="europass">
						<h2><a href="#identification" name="identification">Personas dati</a></h2>
				<p class="vcard">
					<img src="http://a0.twimg.com/profile_images/691424418/apmyp.png" alt="Autoportrait" class="left"/>
					<a class="fn org url" href="http://deatech.lv/index.php?do=feedback" title="Contact Information for DEA Technology/Artūrs"><span class="organization-name">DEA Technology</span>/<span class="organization-unit"><xsl:value-of select="identification/firstname"/>&#160;<xsl:value-of select="/europass/identification/lastname"/></span></a><br/>

					<xsl:if test="//field[@name='step1.addressInfo' and @keep='true']">
					<span class="adr">

						<xsl:if test="//field[@name='step1.application' and @keep='true']">
							<a class="extended-address" href="http://arturs.jansons.net/#about"><xsl:value-of select="application/label"/></a><br/>
						</xsl:if>

						<xsl:for-each select="identification/contactinfo/address/addressLine">
							<span class="street-address"><xsl:value-of select="current()"/></span>
						</xsl:for-each>

						<xsl:if test="string-length(identification/contactinfo/address/municipality) != 0">
							<span class="locality"><xsl:value-of select="identification/contactinfo/address/municipality"/></span>
						</xsl:if>

						<xsl:if test="string-length(identification/contactinfo/address/postalCode) != 0">
							,<span class="postal-code"><xsl:value-of select="identification/contactinfo/address/postalCode"/></span> 
						</xsl:if>
						
						<xsl:if test="string-length(identification/contactinfo/address/country/label) != 0">
							,<b class="country"><xsl:value-of select="identification/contactinfo/address/country/label"/></b>
						</xsl:if>
						<br/>

						<xsl:if test="//field[@name='step1.mobile' and @keep='true']">
							<span class="mob tel">Mob. tālr.:
								<span class="value"><xsl:value-of select="/europass/identification/contactinfo/mobile"/></span>
							</span>
						</xsl:if>

						<xsl:if test="//field[@name='step1.telephone' and @keep='true']">
							,<span class="tel">Tālrunis:
								<span class="value"><xsl:value-of select="identification/contactinfo/telephone"/></span>
							</span>
						</xsl:if>

						<xsl:if test="//field[@name='step1.fax' and @keep='true']">
							,<span class="fax tel">Fakss:
								<span class="value"><xsl:value-of select="identification/contactinfo/fax"/></span>
							</span>
						</xsl:if>
						<br/>
						
						<xsl:if test="//field[@name='step1.email' and @keep='true']">
							<span class="mail">E-pasts:
								<a><xsl:attribute name="href">mailto:<xsl:value-of select="/europass/identification/contactinfo/email"/></xsl:attribute>
									<span class="value"><xsl:value-of select="/europass/identification/contactinfo/email"/></span>
								</a>
							</span>
						</xsl:if>
						<br/>
							...
							<xsl:if test="//field[@name='step1.gender' and @keep='true']">
								<span class="grender">
									<span class="value">
										<xsl:if test="identification/demographics/gender='M'">
											<xsl:text>Vīrietis</xsl:text>
										</xsl:if>
										<xsl:if test="identification/demographics/gender='F'">
							...
							<xsl:if test="//field[@name='step1.birthDate' and @keep='true']">
								<span class="age">
									<!-- <xsl:value-of select="substring(/europass/identification/demographics/birthdate, 9, 2)"/>.<xsl:value-of select="substring(/europass/identification/demographics/birthdate, 6, 2)"/>.<xsl:value-of select="substring(/europass/identification/demographics/birthdate, 1, 4)"/> -->
									(<span class="value"><xsl:value-of select="2008-substring(/europass/identification/demographics/birthdate, 1, 4)"/></span>)
								</span>
							</xsl:if>
						...
						<h2><a href="#workexperience" name="workexperience">Darba pieredze</a></h2>
						<table>
							<xsl:if test="//field[@name='step3List' and @keep='true']">
								<xsl:for-each select="workexperiencelist/workexperience">
									...
									<tr>
										...
										<td><xsl:value-of select="translate(period/from/day,'-','')"/>
											...
											<xsl:value-of select="translate(period/to/year,'-','')"/></td>
									</tr>
									...
									<xsl:if test="//field[@name=concat($indexedStep3,'.position') and @keep='true']">
										<tr>
											...
											<td><xsl:value-of select="position/label"/></td>
										</tr>
									</xsl:if>
									...
									<xsl:if test="//field[@name=concat($indexedStep3,'.company.name') and @keep='true']">
										<tr>
											...
											<td><xsl:value-of select="employer/name"/>
												<xsl:if test="//field[@name=concat($indexedStep3,'.company.addressInfo') and @keep='true']">
													...
													</xsl:if>&#160;(<xsl:value-of select="employer/address/municipality"/>,
									...
									<xsl:if test="//field[@name=concat($indexedStep3,'.company.sector.label') and @keep='true']">
										<tr>
											...
											<td><xsl:value-of select="employer/sector/label"/></td>
										</tr>
									</xsl:if>
								</xsl:for-each>
							</xsl:if>
						...
						<xsl:if test="//field[@name='step4List' and @keep='true']">
							<h2><a href="#education" name="education">Izglītība</a></h2>
							<table>
								<xsl:for-each select="educationlist/education">
									...
									<xsl:if test="//field[@name=concat($indexedStep4,'.educationalOrg.name') and @keep='true']">
										<tr>
											...
											<td><xsl:value-of select="organisation/name"/>
												...
												<xsl:if test="//field[@name=concat($indexedStep4,'.educationalOrg.addressInfo') and @keep='true']">
													...
													</xsl:if><xsl:value-of select="organisation/address/municipality"/>
									...
									<xsl:if test="//field[@name=concat($indexedStep4,'.level.label') and @keep='true']">
										<tr>
											...
											<td><xsl:value-of select="level/label"/></td>
										</tr>
									</xsl:if>
								</xsl:for-each>
							</table>
						</xsl:if><h2><a href="#skilllist" name="skilllist">Prasmes</a></h2>
						<table id="languages">
							<xsl:if test="//field[@name='step5.motherLanguages' and @keep='true']">
								<tr>
									<td class="first">Dzimtā valoda</td>
									<td><xsl:value-of select="/europass/languagelist/language[@type='mother']/label"/></td>
								</tr>
							</xsl:if>
							<xsl:if test="//field[@name='step5.foreignLanguageList' and @keep='true']">
								...
								<xsl:for-each select="/europass/languagelist/language[@type='foreign']">
									...
									<xsl:if test="//field[@name=$indexedLang and @keep='true']">
										<tr>
											<td><xsl:value-of select="label"/></td><td class="indexed"><xsl:value-of select="level/listening"/></td><td>
											...
											<td class="indexed"><xsl:value-of select="level/reading"/></td><td>
											...
											<td class="indexed"><xsl:value-of select="level/spokeninteraction"/></td><td>
											...
											<td class="indexed"><xsl:value-of select="level/spokenproduction"/></td><td>
											...
											<td class="indexed"><xsl:value-of select="level/writing"/></td><td>
							...
							<xsl:if test="//field[@name='step7.additionalInfo' and @keep='true']">
								<tr>
									...
									<td colspan="10"><xsl:value-of select="/europass/misclist/misc[@type='additional']"/></td>
								</tr>
							</xsl:if>
							<xsl:if test="//field[@name='step7.annexes' and @keep='true']">
								<tr>
									<td class="first">Pielikumi</td>
									<td colSpan="10"><xsl:value-of select="europass:learnerinfo/misclist/misc[@type='annexes']"/></td>
								</tr>
							</xsl:if>
						</table><br/>
