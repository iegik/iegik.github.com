...
`});var w=w1(require("fs")),x=(t,e)=>w.writeFileSync(t,e,{encoding:"utf8"}),_=t=>w.readFileSync(t,{encoding:"utf8"});var Z=new Date,C=`v3.1.0-${Z.toJSON()}`,p=btoa(`${Number(Z)}`).slice(10,18);var j=`<svg xmlns="https://www.w3.org/2000/svg" viewBox="0 0 14 14" enable-background="new 0 0 240 240" x="0px" y="0px" xml:space="preserve" fill="currentColor">
...
`;var I=`<svg xmlns="https://www.w3.org/2000/svg" enable-background="new 0 0 55.2 14" version="1.1" viewBox="0 0 55.2 14" xml:space="preserve"  fill="currentColor">
...
`;var N=`<svg xmlns="https://www.w3.org/2000/svg" aria-hidden="true" viewBox="0 0 16 16" data-view-component="true" fill="currentColor">
  <path fill-rule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
</svg>
`;var A=`<svg xmlns="https://www.w3.org/2000/svg" viewBox="0 0 63 18">
...
</svg>`;var q=`<svg viewBox="0 0 62 39" fill="currentColor" xmlns="https://www.w3.org/2000/svg" class="main-header-logo">
...
`,V=v1;var x1={"image/webp":"webp","image/jpeg":"jpg","image/png":"png"},C1={types:"",sizes:"",src:""},k1=({types:t,sizes:e,src:o}=C1)=>Object.entries(x1).filter(([r,i])=>t.includes(i)).flatMap(([r,i])=>`
  ...
  `).join(""),L1=t=>{if(!t)return"";let{src:e,ratio:o,sizes:r,types:i="png|webp",alt:n="",itemprop:s=""}=t,[l,u]=o.split(":"),c=+r.split(",")[0],a=c*+l,h=c*+u;return`
    <picture>
      ${k1({types:i,sizes:r,src:e})}
      <img src="${i.includes("webp")?e.replace(/\..*$/,".webp"):e}" width="${a}" height="${h}" alt="${n}" itemprop="${s}" />
    </picture>
  `},B=L1;var y1=(t={})=>{let{className:e="",children:o,squire:r,tag:i="div"}=t,n="link-list";return e&&(n+=` ${e}`),r&&(n+=" link-list--squire"),`
      <${i} class="${n}">
...
# }`;var J=`query GetUser($login:String!) {
  user(login: $login) {
    avatarUrl
    login
  }
...
`;var b1=async t=>{let{data:e,errors:o}=await t.json();if(o)throw o[0];return e},$=class{constructor(){this.repoName="iegik.github.com";this.repoOwner="iegik";this.storeRoot="gh-pages:data"}static getInstance(){return this.instance}async request({query:e,variables:o}){if(typeof window=="undefined")return;let r=window.sessionStorage?.getItem("access_token"),i=window.sessionStorage?.getItem("token_type"),n=await fetch("https://api.github.com/graphql",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json",Authorization:`${i} ${r}`},body:JSON.stringify({query:e,variables:o})});return b1(n)}async getFile(e){let o=await this.request({query:K,variables:{repo_name:this.repoName,repo_owner:this.repoOwner,object_path:`${this.storeRoot}${e}.json`}});if(!o)throw Error(b);let{repository:{object:r}}=o;if(!r)throw Error(g);let{text:i}=r;return JSON.parse(i)}async getUserInfo(){let{user:e}=await this.request({query:J,variables:{login:this.repoOwner}});if(!e)throw Error(b);return e}},W=$;W.instance=new $;var M1=t=>t.json(),E=class{constructor(){}static getInstance(){return this.instance}async request(e){let o=await fetch(`${location.protocol}//${location.host}/data${e}`,{headers:{"Content-Type":"application/json",Accept:"application/json"}});return M1(o)}async getFile(e){let o=await this.request(`${e}.json`);if(!o)throw Error(g);return o}},Y=E;Y.instance=new E;var S1=({className:t,name:e}={name:"unknown"})=>{let o=f();return setTimeout(()=>{if(typeof window=="undefined"||typeof document=="undefined")return;let r=document.getElementById(`sprite-${e}`)?.getAttribute("viewBox")||"0 0 0 0";o.current?.setAttribute("viewBox",r)}),`
  ...
  `},m=S1;var Me="\u{1F34F},\u{1F330},\u{1F34B},\u{1F345},\u{1F346},\u{1F347},\u{1F353},\u{1F349},\u{1F350},\u{1F352},\u{1F351},\u{1F951}".split(",");var e1=class{constructor(){this.id=null}toString(){let e=btoa(`${Math.ceil(Math.random()*1e13)+ +new Date}`).slice(10,18);return this.id=this.id||`ref-${e}`}get current(){return typeof window=="undefined"||typeof document=="undefined"?null:document.querySelector(`[ref=${this.id}]`)}},f=()=>new e1;var O1=(t={})=>{let{className:e="link",children:o,tag:r="a",to:i="#",title:n="",onClick:s}=t,l=f();return setTimeout(()=>{s&&l.current&&l.current.addEventListener("click",s)}),`
      <${r} class="${e} link" href="${i}" title="${n}" ref="${l}">
        ${o?.join?.("")}
      </${r}>
    `},d=O1;var _1=!1;var M=_1?()=>{}:(...t)=>{console.log(...t)};var i1=r1(),Z1=()=>(setTimeout(()=>{M(i1)}),`
    <header class="header">
      <a href="#home" class="header__avatar" alt="avatar">${B({itemprop:"photo",src:"/images/artursjansons.jpg",ratio:"1:1",sizes:"432,216,64",types:"jpg|png|webp",alt:i1})}</a>
    </header>
  `),n1=Z1;var j1=t=>()=>{if(typeof window=="undefined")return;let e=typeof window=="undefined"?!1:window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches===!0,o,r,i,n,s=16,l,u,c=document?.querySelector(t),a=document?.documentElement,h=c&&a&&Math.max(c.scrollWidth,c.offsetWidth,a.clientWidth,a.scrollWidth,a.offsetWidth),S=c&&a&&Math.max(c.scrollHeight,c.offsetHeight,a.clientHeight,a.scrollHeight,a.offsetHeight);o=r=i=n=0,h&&h>300&&addEventListener("mousemove",function(O){i=O.pageX,n=O.pageY});function a1(){o-=.5,r-=5,!(S==null||h==null)&&(u=`${h/2-2762-i/(5*s)}px ${S-129+(20-n)/(10*s)}px, ${(o-i)/(10*s)}px 60%, ${(r-i)/s}px 40%`)}function R(){a1(),!!c?.style&&(c.style.backgroundPosition=u,l&&window.cancelAnimationFrame(l),l=window.requestAnimationFrame(R))}!e&&R()},I1=()=>(setTimeout(j1("#clouds")),`
    <div id="clouds"></div>
  `),s1=I1;var N1=()=>`
    ${s1()}
    ${n1()}
