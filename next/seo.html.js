#!/usr/bin/env node
"use strict";var u1=Object.create;var D=Object.defineProperty;var w1=Object.getOwnPropertyDescriptor;var v1=Object.getOwnPropertyNames;var C1=Object.getPrototypeOf,y1=Object.prototype.hasOwnProperty;var L1=(t,e)=>()=>(e||t((e={exports:{}}).exports,e),e.exports);var x1=(t,e,o,r)=>{if(e&&typeof e=="object"||typeof e=="function")for(let i of v1(e))!y1.call(t,i)&&i!==o&&D(t,i,{get:()=>e[i],enumerable:!(r=w1(e,i))||r.enumerable});return t};var b1=(t,e,o)=>(o=t!=null?u1(C1(t)):{},x1(e||!t||!t.__esModule?D(o,"default",{value:t,enumerable:!0}):o,t));var h1=L1((he,e2)=>{e2.exports=`O00OO00KXXKKNNXKKXXNNX
...
::lkKXKxlkc,,;:'.l00l,
:0XKOkxokkl.','..dkkk.
`});var b=b1(require("fs")),M=(t,e)=>b.writeFileSync(t,e,{encoding:"utf8"}),X=t=>b.readFileSync(t,{encoding:"utf8"});var q=new Date,S=`v3.1.0-${q.toJSON()}`,d=btoa(`${Number(q)}`).slice(10,18);var z=`<svg xmlns="https://www.w3.org/2000/svg" viewBox="0 0 14 14" enable-background="new 0 0 240 240" x="0px" y="0px" xml:space="preserve" fill="currentColor">
...
`;var U=`<svg xmlns="https://www.w3.org/2000/svg" enable-background="new 0 0 55.2 14" version="1.1" viewBox="0 0 55.2 14" xml:space="preserve"  fill="currentColor">
...
`;var j={"linkedin-icon":z,"linkedin-logo":U,"github-icon":V,"github-logo":B,"stackoverflow-icon":K,"stackoverflow-logo":G,"jsfiddle-icon":J,"jsfiddle-logo":W,"codersrank-icon":Y,"codersrank-logo":Q,"slot-machine-icon":t1};var e1="https://www.w3.org/2000/svg",R=new Map,N1=()=>`
  <svg xmlns="${e1}" display="none">
  ${Object.entries(j).map(([t,e])=>(R.set(t,{viewBox:e.match(/viewBox="([^"]+)"/i)?.[1]}),e.replace(`xmlns="${e1}"`,`id="sprite-${t}"`).replaceAll(/\bsvg\b/g,"symbol"))).join("")}
  </svg>
`,o1=N1;(()=>{if(typeof window>"u")return;document.write=window.write=()=>{},((e,o=[])=>Object.getOwnPropertyNames(e).forEach(r=>{let i=Object.prototype[r];o.includes(r)||Object.getOwnPropertyDescriptor(Object.prototype,r)?.configurable&&Object.defineProperty(Object.prototype,r,{configurable:!1,set:()=>{r!=="trace"&&console.trace(`Property "${r}" was changed in:`)},get:()=>i})}))(Object.prototype,["__proto__"]),window.satismeter=()=>{},window.requestAnimationFrame=window.requestAnimationFrame??(e=>{setTimeout(e)})})();var T1=(t={})=>{let{className:e="",children:o,squire:r,tag:i="div"}=t,n="link-list";return e&&(n+=` ${e}`),r&&(n+=" link-list--squire"),`
      <${i} class="${n}">
        ${o?.map(a=>`<div class="link-list__item">${a}</div>`).join("")}
      </${i}>
    `},O=T1;var A1=(t={})=>{let{className:e="link",children:o,tag:r="a",to:i="#",title:n="",onClick:a}=t,l=u();return setTimeout(()=>{a&&l.current&&l.current.addEventListener("click",a)}),`
      <${r} class="${e} link" href="${i}" title="${n}" ref="${l}" ${/^http/.test(i)?'rel="noopener noreferrer"':""}>
        ${o?.join?.("")}
      </${r}>
    `},g=A1;var w="Page not found",_="Wrong request";var I=!1;var $=I?()=>{}:(...t)=>{console.debug(...t)};var k=I?()=>{}:(...t)=>{console.info(...t)},Z=I?()=>{}:(...t)=>{console.log(...t)};var r1=`query RepoFile($repo_owner: String!, $repo_name: String!, $object_path: String!) {
...
# }`;var i1=`query GetUser($login:String!) {
  user(login: $login) {
    avatarUrl
    login
  }
}
`;var D1=async t=>{let{data:e,errors:o}=await t.json();if(o)throw o[0];return e},n1=class t{constructor(){this.repoName="iegik.github.com";this.repoOwner="iegik";this.storeRoot="gh-pages:data"}static{this.instance=new t}static getInstance(){return this.instance}async request({query:e,variables:o}){if(typeof window>"u")return;let r=window.sessionStorage?.getItem("access_token"),i=window.sessionStorage?.getItem("token_type"),n=await fetch("https://api.github.com/graphql",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json",Authorization:`${i} ${r}`},body:JSON.stringify({query:e,variables:o})});return D1(n)}async getFile(e){let o=await this.request({query:r1,variables:{repo_name:this.repoName,repo_owner:this.repoOwner,object_path:`${this.storeRoot}${e}.json`}});if(!o)throw Error(_);let{repository:{object:r}}=o;if(!r)throw Error(w);let{text:i}=r;return JSON.parse(i)}async getUserInfo(){let{user:e}=await this.request({query:i1,variables:{login:this.repoOwner}});if(!e)throw Error(_);return e}};var z1=t=>t.json(),a1=class t{static{this.instance=new t}constructor(){}static getInstance(){return this.instance}async request(e){let o=await fetch(`${location.protocol}//${location.host}/data${e}`,{headers:{"Content-Type":"application/json",Accept:"application/json"}});return z1(o)}async getFile(e){let o=await this.request(`${e}.json`);if(!o)throw Error(w);return o}};var T=class{constructor(){this.id=null}toString(){let e=btoa(`${Math.ceil(Math.random()*1e13)+ +new Date}`).slice(10,18);return this.id=this.id||`ref-${e}`}get current(){return typeof window>"u"||typeof document>"u"?null:document.querySelector(`[ref=${this.id}]`)}},u=()=>new T;var V1=({className:t,name:e}={name:"unknown"})=>{let o=u();return setTimeout(()=>{if(typeof window>"u"||typeof document>"u")return;let r=document.getElementById(`sprite-${e}`)?.getAttribute("viewBox")||"0 0 0 0";o.current?.setAttribute("viewBox",r)}),`
    <svg class="${t}" preserveAspectRatio="xMidYMid meet" viewBox="${R.get(e)?.viewBox||"0 0 0 0"}" ref="${o}">
      <use xlink:href="#sprite-${e}" />
    </svg>
  `},m=V1;var E=0,A=!1,L="",c1=[],F="\u{1F34F},\u{1F330},\u{1F34B},\u{1F345},\u{1F346},\u{1F347},\u{1F353},\u{1F349},\u{1F350},\u{1F352},\u{1F351},\u{1F951}".split(","),{floor:B1,random:K1}=Math,G1=t=>t[B1(K1()*t.length)],P=()=>{if(A)return L;let[t,e,o]=[F,F,F].flatMap(G1);if(L=`${t} ${e} ${o}`,$(`${E} ${L}`),t==e&&e==o){let r=`
            ${L}
    You won in ${E} tries`;A=!0,c1.push(()=>{setTimeout(()=>{k(r),alert(r),A=!1})}),E=0}else E++;return L},J1=()=>{let t=u();return setTimeout(()=>{if(typeof window>"u")return;let e=new MutationObserver((o,r)=>{for(let i of o)if(i.type==="childList"){let n=c1.pop();n&&n()}});t.current&&e.observe(t.current,{childList:!0}),t.current?.addEventListener("click",()=>{t.current&&(t.current.innerHTML=P())}),document.addEventListener("keydown",o=>{o.isComposing||o.keyCode===229||o.which!==13&&o.which!==32||t.current&&(t.current.innerHTML=P())})}),`
    <div class="slot-machine">
      ${m({name:"slot-machine-icon",className:"slot-machine__icon"})}
      <span class="slot-machine__reels" ref="${t}">${P()}</span>
    </div>
  `},N=J1;var W1={"image/webp":"webp","image/jpeg":"jpg","image/png":"png"},Y1={types:"",sizes:"",src:""},Q1=({types:t,sizes:e,src:o}=Y1)=>Object.entries(W1).filter(([r,i])=>t.includes(i)).flatMap(([r,i])=>`
    <source type="${r}" srcset="${e.split(",").map(n=>o.replace(new RegExp(`.(${t})$`),`_${n}.${i} ${n}w`))}" />
  `).join(""),t2=t=>{if(!t)return"";let{src:e,ratio:o,sizes:r,types:i="png|webp",alt:n="",itemprop:a=""}=t,[l,h]=o.split(":"),c=+r.split(",")[0],s=c*+l,p=c*+h;return`
    <picture>
      ${Q1({types:i,sizes:r,src:e})}
      <img src="${i.includes("webp")?e.replace(/\..*$/,".webp"):e}" width="${s}" height="${p}" alt="${n}" itemprop="${a}" />
    </picture>
  `},l1=t2;async function p1(t){let e=new Image;e.crossOrigin="Anonymous",e.src=t,await e.decode();let o=e.width,r=e.height,n=new OffscreenCanvas(o,r).getContext("2d");if(!n)return[];n.drawImage(e,0,0,o,r);let a=n.getImageData(0,0,o,r).data,l="\u2802\u2802\u2802\u2802\u2801\u2880\u2881\u2881\u288C\u288C\u2895\u2895\u2897\u2897\u289F\u289F\u28B7\u28B7\u28BF\u28BF",h=[],c=[];for(let s=0;s<r;s++){for(let p=0;p<o;p++){let f=(s*o+p)*4,x=a[f],v=a[f+1],C=a[f+2];if(a[f+3]===0)continue;let f1=(x+v+C)/3,H=Math.floor((1-f1/255)*10);h.push(l[H*2]+l[H*2+1]),c.push(`#${x.toString(16).padStart(2,"0")}${v.toString(16).padStart(2,"0")}${C.toString(16).padStart(2,"0")}`)}h[h.length-1]+=`
`}return[h.map(s=>`%c${s}`).join(""),...c.map(s=>`background-color: ${s};`)]}var o2=h1(),r2=()=>(setTimeout(async()=>{if(typeof window>"u"||typeof document>"u")return;let t=await p1("images/artursjansons_32.jpg");Z(...t)}),`
    <header class="header">
      <a href="#/home" class="header__avatar" alt="avatar">${l1({itemprop:"photo",src:"/images/artursjansons.jpg",ratio:"1:1",sizes:"432,216,64",types:"jpg|png|webp",alt:o2})}</a>
    </header>
  `),d1=r2;var i2=t=>()=>{if(typeof window>"u")return;let e=typeof window>"u"?!1:window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches===!0,o,r,i,n,a=16,l,h,c=document?.querySelector(t),s=document?.documentElement,p=c&&s&&Math.max(c.scrollWidth,c.offsetWidth,s.clientWidth,s.scrollWidth,s.offsetWidth),f=c&&s&&Math.max(c.scrollHeight,c.offsetHeight,s.clientHeight,s.scrollHeight,s.offsetHeight);o=r=i=n=0,p&&p>300&&addEventListener("mousemove",function(C){i=C.pageX,n=C.pageY});function x(){o-=.5,r-=5,!(f==null||p==null)&&(h=`${p/2-2762-i/(5*a)}px ${f-129+(20-n)/(10*a)}px, ${(o-i)/(10*a)}px 60%, ${(r-i)/a}px 40%`)}function v(){x(),c?.style&&(c.style.backgroundPosition=h,l&&window.cancelAnimationFrame(l),l=window.requestAnimationFrame(v))}!e&&v()},n2=()=>(setTimeout(i2("#clouds")),`
    <div id="clouds"></div>
  `),m1=n2;var s2=()=>`
    ${m1()}
    ${d1()}
    <article lang="en">
      <section id="home" itemscope itemtype="https://schema.org/Person">
        <header>
          ...
          ${O({squire:!0,children:[g({to:"https://linkedin.com/in/iegik",title:"LinkedIn",children:[m({name:"linkedin-icon",className:"link-list__icon"})]}),g({to:"https://github.com/iegik",title:"GitHub",children:[m({name:"github-icon",className:"link-list__icon"})]}),g({to:"https://profile.codersrank.io/user/iegik",title:"Coders Rank",children:[m({name:"codersrank-icon",className:"link-list__icon"})]}),g({to:"https://jsfiddle.net/user/iegik",title:"JSFiddle",children:[m({name:"jsfiddle-icon",className:"link-list__icon"})]}),g({to:"https://stackoverflow.com/users/771471/iegik",title:"Stack Overflow",children:[m({name:"stackoverflow-icon",className:"link-list__icon"})]})]})}
  ...
  `,g1=s2;var a2=X("./public/next/styles.min.css"),c2=Object.entries({"script-src":[`'nonce-${d}'`,"'strict-dynamic'","https://www.google-analytics.com","https://ssl.google-analytics.com","https://static.hotjar.com","https://js.sentry-cdn.com","'unsafe-inline'"],"img-src":["'self'","https://www.google-analytics.com","https://avatars.githubusercontent.com/","https://www.googletagmanager.com/"],"connect-src":["'self'","https://www.google-analytics.com","https://region1.google-analytics.com","https://in.hotjar.com","https://static.hotjar.com","https://github.com/login/oauth/access_token","https://api.github.com/graphql","https://api.github.com/user","https://qilg4ch66b3vpgtevzccb5meum0ttfcl.lambda-url.eu-north-1.on.aws/","https://o171820.ingest.sentry.io/"],"style-src":[`'nonce-${d}'`,"https://static.hotjar.com"],"object-src":["'none'"],"base-uri":["'none'"],"frame-src":["https://vars.hotjar.com/"]}).reduce((t,[e,o])=>`${t};${e} ${o.join(" ")}`,"default-src 'self'"),l2="Art\u016Brs Jansons :: Web Developer",p2="Experienced web developer with a passion for innovation, automation and optimization",h2=`<!DOCTYPE html>
<html lang="en" manifest="manifest.appcache">
  <head>
    <meta http-equiv="Content-Security-Policy" content="${c2}">
  ...
  <body>
    ...
    <script async nonce="${d}" src="https://www.googletagmanager.com/gtag/js?id=G-5ZY8Y6X2C4"></script>
    <script nonce="${d}">
    ...
    <script nonce="${d}" src="https://browser.sentry-cdn.com/7.54.0/bundle.min.js" integrity="sha384-EmlJLN9Q0yu0/2UUCIYnEM88jpQ7xUhtNI2ZeXb/ci3cwoAoIQl350N4PQPlMbP5" crossorigin="anonymous"></script>
    <script nonce="${d}">
...
`;M("public/next/index.html",h2);var d2=`
CACHE MANIFEST
# rev ${S}
...
NETWORK:
*
`;M("public/next/manifest.appcache",d2);
